...
$config_files{yaml}  = { filename => write_config( $yaml, 'yaml' ), obj => $yaml };
$yaml->{_config_path} = $cwd . '/' . $config_files{yaml}->{filename};

$config_files{json}  = { filename => write_config( $json, 'json' ), obj => $json };
...
if ( !$config_path) {
  ...
  BAIL_OUT("coud not find $config_path/tagx.xml");
}
